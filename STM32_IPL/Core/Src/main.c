...
// STM32 IPL buffer
uint8_t stm32iplBuffer[STM32IPL_INT_BUFFER_SIZE] __attribute__((aligned(32), section(".sdram_bank2")));
...
int main(void)
{
	...
	// explore driver
	BSP_CAMERA_SetBrightness(0, 0);
	int32_t brightness;
	BSP_CAMERA_GetBrightness(0, &brightness);
	uint32_t PixelFormat;
	BSP_CAMERA_GetPixelFormat(0, &PixelFormat);

	BSP_CAMERA_SetPixelFormat(0, CAMERA_PF_RGB565);
	uint32_t pixelFormatRead;
	BSP_CAMERA_GetPixelFormat(0, &pixelFormatRead);

	BSP_CAMERA_SetResolution(0, CAMERA_R640x480);
	uint32_t Resolution;
	BSP_CAMERA_GetResolution(0, &Resolution);

	// int32_t ret = BSP_CAMERA_SetMirrorFlip(0, OV5640_MIRROR_FLIP);
	uint32_t MirrorFlip;
	BSP_CAMERA_GetMirrorFlip(0, &MirrorFlip);

	BSP_CAMERAEx_SetTiming(0, CAMERA_TIMING_12FPS);

	// BSP_CAMERA_SetColorbarModeConfig(0, COLORBAR_MODE_ENABLE);
	...
	stm32ipl_err_t resultIpl;

	image_t imgSrc;
	STM32Ipl_Init(&imgSrc, MAX_WIDTH, MAX_HEIGHT, IMAGE_BPP_RGB565, pCopiedBuff);

	image_t imgDst;
	resultIpl = STM32Ipl_AllocData(&imgDst, MAX_WIDTH, MAX_HEIGHT, IMAGE_BPP_RGB565);
	if (resultIpl != stm32ipl_err_Ok)
	{
		Error_Handler();
	}

	image_t imgDst2;
	resultIpl = STM32Ipl_AllocData(&imgDst2, MAX_WIDTH, MAX_HEIGHT, IMAGE_BPP_RGB565);
	if (resultIpl != stm32ipl_err_Ok)
	{
		Error_Handler();
	}
	...
	while (1)
	{
		...
		else if (camera_state == CAM_STATE_CONFIGURED)
		{
			...
			for (uint32_t i = 0; i < camera_buf_size / sizeof(uint16_t); i++)
			{
				//*(((uint16_t *) pCopiedBuff) + i) = (uint16_t) 0b1111000000000000;// *(((uint16_t *) pCameraBuff) + i);

				*(((uint16_t *)pCopiedBuff) + i) = *(((uint16_t *)pCameraBuff) + i);
			}

			// memcpy(pCopiedBuff, pCameraBuff, sizeof(pCameraBuff));
			...
			resultIpl = STM32Ipl_CopyData(&imgSrc, &imgDst);
			if (resultIpl != stm32ipl_err_Ok)
			{
				Error_Handler();
			}

			// STM32Ipl_Binary(&imgDst, &imgDst, &thresholds, true, false, NULL);

			// STM32Ipl_MeanFilter(&imgSrc, 3, false, 0, false, NULL);

			// resultIpl = STM32Ipl_CopyData(&imgDst, &imgSrc);
			// if (resultIpl != stm32ipl_err_Ok)
			// {
			// 	Error_Handler();
			// }

			// STM32Ipl_ReleaseData(&imgDst);
			// STM32Ipl_ReleaseData(&imgDst2);
