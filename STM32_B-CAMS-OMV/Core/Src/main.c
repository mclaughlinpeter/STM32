...
int main(void)
{
	...
	// explore ISP functions
	int32_t ret;

	ret = BSP_CAMERA_SetBrightness(0, 0);
	
	ret = BSP_CAMERA_SetMirrorFlip(0, OV5640_MIRROR_FLIP);
	
	// ret = BSP_CAMERA_SetColorbarModeConfig(0, COLORBAR_MODE_ENABLE);
		...
		else if (camera_state == CAM_STATE_CONFIGURED)
		{
			...
			{			
				*(((uint16_t *)pCopiedBuff) + i) = *(((uint16_t *)pCameraBuff) + i);
				...
				/* Demonstrate RGB565 */
				// *(((uint16_t *) pCopiedBuff) + i) = (uint16_t) 0b1111100000000000;
			}			

			if (camera_format == CAMERA_PF_RGB565)
			{
				/* Transform color space to YCbCr */
